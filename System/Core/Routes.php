...
class Routes {
	...
	public static function name($name = '', $value = null) {
		if (is_array($name)) {
			...
			return isset(self::$rules['name'][$name]) ?: null;
		}
	}	
	...
	public static function rule($rule, $route = '', $type = "*", $option = [], $patten = []) {
		...
		if (is_array($rule) && empty($route)) {
			foreach ($rule as $key => $value) {
				...
				if (is_array($value)) {
					$route = $value[0];
					$option1 = array_merge($option, isset($value[1]) ?: []);
					$patten1 = array_merge($patten, isset($value[2]) ?: []);
				} else {
					$option1 = null;
					$patten1 = null;
					$route = $value;
				}
				self::setRoute($key, $route, $type, !is_null($option1) ?: $option, !is_null($patten1) ?: $patten, $group);
			}
		} else {
		...
		if (!empty($name)) {
			if ($routes instanceof \Closure) {
				...
				foreach ($routes as $key => $value) {
					...
					$options = isset($option1) ?: $option;
					$pattens = isset($patten1) ?: $patten;
					if ('$' == substr($key, -1, 1)) {
